...
var body: some View {
    NavigationStack {
        ScrollView {
            VStack(spacing: 16) {
                IconTitle(systemName: "location.circle.fill",
...
VStack(spacing: 12) {
    LabeledField(title: "Street", placeholder: "1600 Amphitheatre Parkway", text: $street)
        HStack(spacing: 12) {
            LabeledField(title: "City", placeholder: "Mountain View", text: $city)
                LabeledField(title: "State", placeholder: "CA", text: $state, textCase: .characters)
                    .frame(width: 100)
            }
            HStack(spacing: 12) {
            LabeledField(title: "ZIP Code", placeholder: "94043", text: $postalCode, keyboard: .numbersAndPunctuation)
            LabeledField(title: "Country", placeholder: "US", text: $countryCode, textCase: .characters)
                    .frame(width: 100)
                }
            }
        }
...
if showingResult {
    ...
    ResultCard(title: statusText(for: response), subtitle: nil) {
                                ...
                                ForEach(Array(candidates.enumerated()), id: \.offset) { _, candidate in
                                    if let address = candidate.addressKeyFormat {
                                        HStack(alignment: .firstTextBaseline) {
                                            Image(systemName: "location.fill").foregroundColor(Theme.Colors.primary)
                                            ...
                                            if let classification = candidate.addressClassification {
                                                HStack(spacing: 6) {
                                                Image(systemName: "tag.fill").font(.caption2).foregroundColor(Theme.Colors.secondaryText)
        ...
        .toolbar { ToolbarItem(placement: .topBarTrailing) { Button("Sample") { fillSampleAddress() }.font(Theme.Typography.caption) } }
}
}
...
if response.isValid { return "Address Validated" }
else if response.isAmbiguous { return "Similar Addresses Found" }
