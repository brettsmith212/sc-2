...
extension NSNotification.Name {
    static let addressSaved = NSNotification.Name("addressSaved")
}
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    // Then dismiss the AddressValidationView to get back to AddressBook
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                        // Use notification to tell parent views to dismiss
...
struct SaveAddressSheetForNavigation: View {
    let candidate: AddressCandidate
